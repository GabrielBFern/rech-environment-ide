...
export default class Open {
  ...
  abreArquivoCursor() {
    ...
    // Se for um comando Winmerge
    if (editor.getCurrentLine().trim().toUpperCase().startsWith("WINMERGE")) {
       // Abre WinMerge removendo possíveis espaços duplicados
       new Exec().exec(editor.getCurrentLine().trim().replace(/\s+/g,' '));
    }
  }
