...
export default class Busca {
  ...
  activate(rechModule) {
    rechModule.addCommands({
      ...
      'rech:busca-proximo-paragrafo': () => this.buscaProximoParagrafo(),
      'rech:busca-paragrafo-anterior': () => this.buscaParagrafoAnterio()
    });
  }
   ...
   */
  encontraDeclaracao(termo, buscaPrimeiroNoEditor, path, podePreprocessar) {
    // TODO:
      /*
      ...
      */
      return new Promise((resolve, reject) => {
        ...
        if (buscaPrimeiroNoEditor) {
          var result = this.encontraDeclaracaoBuffer(termo, path, new Editor().getEditorBuffer());
          if (result != null) {
            resolve(result);
            return;
          }
        ...
        if (podePreprocessar) {
          this.encontraDeclaracaoPreproc(termo, path, "is", true).then((result) => {
            resolve(result);
          }).catch((e) => {
            reject(e);
          });
   ...
   */
  encontraDeclaracaoBuffer(termo, path, buffer) {
    var editor = new Editor();
    ...
    new Scan(buffer).scan(new RegExp(termo, 'g'), (iterator) => {
      if (parser.isDeclaracao(termo, iterator.lineContent)) {
        result = [path.fullPath(), iterator.row, iterator.column];
        iterator.stop();
      }
    });
    return result;
   ...
   */
  encontraDeclaracaoPreproc(termo, path, compilador, cache) {
    return new Promise((resolve, reject) => {
      ...
      if (cache) {
        ...
        this.encontraDeclaracaoFilePreproc(termo, path, cacheFileName).then((result) => {
          ...
          this.encontraDeclaracaoPreproc(termo, path, "is", false).then((result) => {
            resolve(result);
          }).catch((e) => {
            reject(e);
          });
        ...
        preproc.exec().then(() => {
          this.encontraDeclaracaoFilePreproc(termo, path, cacheFileName).then((result) => {
            ...
            if (compilador == "is") {
              ...
              this.encontraDeclaracaoPreproc(termo, path, "mf", false).then((result) => {
                resolve(result);
              }).catch((e) => {
                reject(e);
              });
...
*/
  ...
  /**
   * Busca o próximo parágrafo
   */
  buscaProximoParagrafo() {
    var editor = new Editor();
    editor.scan(/^\s{7}[\w\-]+\./g, function(iterator) {
      if (iterator.range.start.row > editor.getCurrentRow()) {
        editor.setCursor([iterator.range.start.row, 7]);
        iterator.stop();
      }
    });
  }
  ...
  /**
   * Busca o parágrafo anterior
   */
  buscaParagrafoAnterio() {
    var editor = new Editor();
    var range = undefined;
    editor.scan(/^\s{7}[\w\-]+\./g, function(iterator) {
      if (iterator.range.start.row < editor.getCurrentRow()) {
        range = [iterator.range.start.row, 7];
      } else {
        iterator.stop();
      }
    });
    if (range !== undefined) {
      editor.setCursor(range);
    }
  }
