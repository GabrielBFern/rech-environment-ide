...
export default class ParserCobol {
  ...
  /**
   * Retorna o texto do comentário da linha
   */
  getTextoComentario(line) {
    if (!this.isComentario(line)) {
      return "";
    }
    line = line.trim();
  ...
  /**
   * Retorna se a linha é um comando copy
   */
  isLinhaCopy(line) {
    return /^\s+COPY\s+\w+\.\w+.*/.test(line);
  }

  /**
   * Retorna o nome do copy na linha corrente
   */
  getNomeCopy(line) {
    return /^\s+COPY\s+(\w+\.\w+).*/.exec(line)[1];
  }
