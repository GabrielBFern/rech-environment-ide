...
export default class Comentarios {
  ...
  puxaComentarioCopy(parser, gerador, editor) {
    ...
    editor.findFilePath(nomeCopy).then((path) => {
      let comentario = this.extraiComentarioCopy(path.fullPath());
   ...
   */
  extraiComentarioCopy(nomeCopy) {
    return nomeCopy;
  }

/*
'Extrai comentário de copy
Function extraiComentCOPY(nomeCopy)
  'Pega a referência ao editor ativo
   set editor = newEditor()
   editor.assignActiveEditor
  'Tenta abrir primeiro no diretório corrente
   diretorio = getVarValue("%FilePath%")
  'Se o arquivo não existe
   if not FunArquivo.fileExist(diretorio & nomeCopy) then
     'Tenta no diretório de fontes
      diretorio = FunArquivo.getDirFontes()
     'Se o arquivo não existe
      if not FunArquivo.fileExist(diretorio & nomeCopy) then
         MsgBox "Não foi possível encontrar o COPY " & nomeCopy & "!"
         Exit Function
      end if
   end if
  'Carrega as 10 primeiras linhas do copy
   lines = Split(FunArquivo.carregaBufferIntervalo(diretorio & nomeCopy, 0, 10), Funcoes.enter())
   lendoComent = false
  'Inicializa o comentário
   comentario = ""
  'Percorre as linhas
   for each line in lines
     'Se for uma linha de divisão
      if FunInterp.isDivisao(line) then
        'Se não estava lendo o comentário
         if not lendoComent then
           'Indica que está lendo
            lendoComent = true
         else
            exit for
         end if
      else
        'Se está lendo o comentário
         if lendoComent then
           'Extrai o texto do comentário
            line = FunInterp.getComentario(line)
           'Recomenta a linha
            line = FunInterp.comenta(line)
           'Usa a linha de comentário
            comentario = line
           'Se for copy de Working/FD/Select
            if (Mid(nomeCopy, 3, 3) = "REG" or Mid(nomeCopy, 3, 3) = "ARQ") and _
               (Mid(nomeCopy, 2, 1) = "W" or Mid(nomeCopy, 2, 1) = "F" or Mid(nomeCopy, 2, 1) = "S") then
              'Se tem o cabeçalho padrão
               if InStr(line, "Descrição:") > 0 then
                 'Se for working
                  if Mid(nomeCopy, 2, 1) = "W" then
                     tipo = "Registro de Working"
                  end if
                 'Se for FD
                  if Mid(nomeCopy, 2, 1) = "F" then
                     tipo = "Registro de FD"
                  end if
                 'Se for Select
                  if Mid(nomeCopy, 2, 1) = "S" then
                     tipo = "Select"
                  end if
                 'Monta o comentário
                  comentario = "      *>-> " & tipo & " do arquivo de " & Trim(Mid(line, 23, Len(line) - 21))
               end if
            end if
           'Encerra o loop
            exit for
   ...
   next
   extraiComentCOPY = comentario & Chr(13) & Chr(10)
End Function
*/
